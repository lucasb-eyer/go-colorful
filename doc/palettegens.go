...
func main() {
    ...
    if err != nil {fmt.Printf("Error generating warm palette: %v", err); return}
    fwarm := colorful.FastWarmPalette(colors)
    happy, err := colorful.HappyPalette(colors)
    if err != nil {fmt.Printf("Error generating happy palette: %v", err); return}
    fhappy := colorful.FastHappyPalette(colors)
    soft, err := colorful.SoftPalette(colors)
    if err != nil {fmt.Printf("Error generating soft palette: %v", err); return}
    brownies, err := colorful.SoftPaletteEx(colors, colorful.SoftPaletteSettings{isbrowny, 50, true})
    if err != nil {fmt.Printf("Error generating brownies: %v", err); return}
    for i := 0 ; i < colors ; i++ {
