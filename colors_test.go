...
func TestIssue11(t *testing.T) {
	...
	blend = c1.BlendLrgb(c2, 0).Hex()
	if blend != c1hex {
		t.Errorf("Issue11: %v --Lrgb-> %v = %v, want %v", c1hex, c2hex, blend, c1hex)
	}
	blend = c1.BlendLrgb(c2, 1).Hex()
	if blend != c2hex {
		t.Errorf("Issue11: %v --Lrgb-> %v = %v, want %v", c1hex, c2hex, blend, c2hex)
	}
